...
namespace OoOVisual
{
    namespace Visualizer
    {
		...
		void Draw_Element_Reorder_Buffer::show_detailed() {
			...
			ImGui::SetNextWindowPos(ImVec2{io.DisplaySize.x - sidebar_width, 0.0f}, ImGuiCond_Always);
			...
			if (ImGui::Begin("ROB Status", nullptr, window_flags)) {
				ImGui::Text("Reorder Buffer State");
				...
				if (ImGui::BeginTable("ROB_Scroll_Table", 5, flags, ImGui::GetContentRegionAvail())) {
					...
					ImGui::TableSetupColumn("Ready", ImGuiTableColumnFlags_WidthFixed, 45.0f);
					ImGui::TableSetupColumn("Details", ImGuiTableColumnFlags_WidthFixed, 300.0f); 
					ImGui::TableSetupColumn("Ptr", ImGuiTableColumnFlags_WidthFixed, 45.0f);
					...
					for (int i{0}; i < Core::Constants::REORDER_BUFFER_SIZE; ++i) {
						...
						ImGui::TableSetColumnIndex(1); if(entry) ImGui::Text("%s", Utils::flowtype_to_str(entry->flow_type));
						ImGui::TableSetColumnIndex(2); if(entry) ImGui::Text(entry->ready ? "YES" : "NO");
						...
						if (is_head) ImGui::Text("HEAD");
						if (is_tail) ImGui::Text("TAIL");
					}
					ImGui::EndTable();
				}
