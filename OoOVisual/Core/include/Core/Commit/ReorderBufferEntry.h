...
namespace OoOVisual
{
	namespace Core {
		struct Reorder_Buffer_Entry {
			...
			Reorder_Buffer_Entry(FrontEnd::FLOW_TYPE flow,u32 origin_branch_timestamp_, time_t self_timestamp_) : flow_type(flow), origin_branch_timestamp(origin_branch_timestamp_), self_timestamp(self_timestamp_) {}
		...
		struct Register_Reorder_Buffer_Entry : Reorder_Buffer_Entry { // load dispatching and jump will also create this type of entry
			...
			Register_Reorder_Buffer_Entry(
				...
				Reorder_Buffer_Entry(flow,origin_branch_timestamp,self_timestamp_) {}
		};
		struct Store_Reorder_Buffer_Entry : Reorder_Buffer_Entry {
			u32 store_id;
			Store_Reorder_Buffer_Entry(FrontEnd::FLOW_TYPE flow, u32 id_, time_t origin_branch_timestamp, time_t self_timestamp) : store_id(id_), Reorder_Buffer_Entry(flow,origin_branch_timestamp,self_timestamp) {}
		};
		...
		struct Branch_Unconditional_Reorder_Buffer_Entry : Reorder_Buffer_Entry {
			...
			Branch_Unconditional_Reorder_Buffer_Entry ( 
				...
				Reorder_Buffer_Entry(flow,origin_branch_timestamp_,self_timestamp_) {}
